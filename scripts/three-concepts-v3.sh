#!/bin/bash
# Render music-forward v3 reviews for the three approved ChatGPT concepts.
...
render_one() {
  ...
  local wav="${TMPDIR:-/tmp}/$slug-v3-$$.wav"
  local video="$DEST/$slug-v3-review.mp4"
  mkdir -p "$DEST"
...
echo "Music-forward v3 review batch complete: $DEST"
