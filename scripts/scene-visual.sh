...
if [ -f "$FOG_OVERLAY" ] && [ -f "$RAIN_OVERLAY" ]; then
  ...
  FOG_FILTER="[1:v]setpts=PTS/${VISUAL_FOG_PLAYBACK_SPEED},fps=12,scale=760:720:force_original_aspect_ratio=increase,crop=640:720:x='60+35*sin(2*PI*t/17)',eq=saturation=0:contrast=1.25:brightness=-0.02,curves=all='0/0 0.18/0 0.42/0.20 0.72/0.68 1/0.92',format=gbrp[fog]"
  RAIN_FILTER="[2:v]setpts=PTS/${VISUAL_RAIN_PLAYBACK_SPEED},fps=12,scale=640:720:force_original_aspect_ratio=increase,crop=640:720,eq=saturation=0:contrast=1.35:brightness=0.015,curves=all='0/0 0.08/0 0.28/0.16 0.62/0.72 1/1',format=gbrp[rain]"
...
ffmpeg -y -nostdin \
  ...
  -filter_complex "
    ...
    [rightsrc]scale=2560:1440:force_original_aspect_ratio=increase,crop=2560:1440,
      ...
      eq=contrast='1.035+0.008*sin(2*PI*n/420)':brightness='-0.018+${VISUAL_LIGHT_BREATHE}*sin(2*PI*n/216)',vignette=angle=0.42,format=gbrp[rightbase];
    ${FOG_FILTER};
    ${RAIN_FILTER};
